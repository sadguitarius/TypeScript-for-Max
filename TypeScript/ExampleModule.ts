...
export function pixelFn(
	...
	r +=
		Math.sin(t + x * Math.sin(t + x * 0.02) * prmA) +
		Math.cos(t + y * (Math.cos(t * 0.7) * 5.0)) * prmB;
	return colorFn(r + t);
}
...
export class TheClass {
	...
	constructor(index: number) {
		this.index = index;
		post(
			`TheClass.constructor was called with the number parameter ${index}`
		);
	}
